...
const RouteBuilderScreen = () => {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        setSelectedSequence(sequenced.filter(id => id !== undefined && id !== null));
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      }).filter(s => s && s.customerId !== undefined && s.customerId !== null);
  ...
  const displayList = useMemo(() => {
    ...
    const safeSequence = Array.isArray(selectedSequence) ? selectedSequence : [];

    const selected = safeSequence
      .map(id => safeCustomers.find(c => c && String(c.id !== undefined && c.id !== null ? c.id : c._id) === String(id)))
      .filter(Boolean);

    const unselected = safeCustomers.filter(
      c => c && !safeSequence.some(seqId => String(seqId) === String(c.id !== undefined && c.id !== null ? c.id : c._id))
    );
